...
class session1:
    def __init__(self, bind="", proxy="", worker="", force_tls1_2=False, cloud=False):
        ...
        self.ssl_version = ssl.PROTOCOL_TLSv1_2 if force_tls1_2 else None

        # Set pool manager options if necessary (curl_cffi typically handles it differently)
        self._init_curl_options(source_address, socket_options)

    def _init_curl_options(self, source_address, socket_options):
        """Initialize curl options for network settings."""
        # curl_cffi manages many settings internally, including socket binding.
        # Direct socket handling isn't natively supported like requests.
        if source_address != ("", 0):
            self.session.curl.setopt(self.session.curl.IPRESOLVE, source_address)
