...
router.get(
  ...
  async (context) => {
    ...
    if (context.params.playlist == "index.m3u8") {
      ...
      const cors = context.request.url.searchParams.get("cors");
      const childQuery = cors ? `?cors=${cors}` : "";
      const playlist = [];
      ...
      for (const channel in ch) {
        ...
        playlist.push(
          `http://${context.request.url.host}${modulePrefix}/live/${channel}/index.m3u8${childQuery}`,
        );
      }
