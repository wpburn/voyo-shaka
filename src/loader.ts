  ...
  try {
    ...
    if (initData.includes("#EXTM3U")) {
      const initm3u8 = initData.includes("\n")
        ? initData
        : (initData.split(" ")).join("\n");
      const corsPrefix = `http://127.0.0.1:${LOCAL_PORT}/cors/`;
      const stripCors = (u: string) =>
        u.startsWith(corsPrefix) ? u.slice(corsPrefix.length) : u;
      if (initm3u8.includes(".m3u8")) {
        const masterParent = stream.stream.match(/(.*)\/.*/)?.[1] || "";
        const variantFetchUrl = m3u8Select(
          initm3u8,
          cors ? `${corsPrefix}${masterParent}` : masterParent,
        );
        const q_m3u8 = await axios.get(variantFetchUrl);
        const variantUrl = stripCors(variantFetchUrl);
        const variantBase = variantUrl.match(/(.*)\/.*/)?.[1] || "";
        const finalP = m3uFixURL(
          q_m3u8.data,
          cors ? `${corsPrefix}${variantBase}` : variantBase,
        );
        return finalP;
      } else {
