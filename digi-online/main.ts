...
const DIGI_API_BASE = "https://digiapis.rcs-rds.ro/digionline/api/v13";
const DIGI_AUTHORIZATION = "Basic YXBpLXdzLXdlYmFkbWluOmRldl90ZWFtX3Bhc3M=";
const USER_AGENT = "okhttp/4.8.1";
const DEVICE_MODEL = "iptvro";
const DEVICE_MAKER = "Kodeative";
const DEVICE_OS = "REL_12";
const DIGI_APP_MARKER = `${DEVICE_MAKER}${DEVICE_MODEL}${DEVICE_OS}`;
const STREAM_PLATFORM = "iOS";
const STREAM_VERSION_APP = "release";
const STREAM_SOURCE = "app";
const STREAM_QUALITY = "all";
...
const config: DigiOnlineConfig = {
  user: env("DIGI_ONLINE_USER"),
  pass: env("DIGI_ONLINE_PASS"),
  port: Number(env("DIGI_ONLINE_PORT", "8093")) || 8093,
  cdmUrl: env("DIGI_ONLINE_CDM_URL", "http://127.0.0.1:8091").replace(
    /\/$/,
    "",
  ),
  mp4decrypt: resolveExecutable(env("DIGI_ONLINE_MP4DECRYPT", "mp4decrypt")),
  shakaPackager: resolveExecutable(
...
function md5(input: string): string {
  ...
  const rotl = (value: number, shift: number) =>
    ((value << shift) | (value >>> (32 - shift))) >>> 0;
  for (let offset = 0; offset < buffer.length; offset += 64) {
    const words = Array.from(
      { length: 16 },
      (_, i) => view.getUint32(offset + i * 4, true),
    );
    let a = a0;
    let b = b0;
    let c = c0;
...
function uuidGen(count: number, value = ""): string {
  if (count === 0) return value;
  return uuidGen(count - 1, value + crypto.randomUUID().replaceAll("-", ""));
}

function generateDeviceRegistration(
  username: string,
  passwordHash: string,
  userHash: string,
): { deviceId: string; registrationHash: string } {
  const devicePrefix = `${DEVICE_MAKER}_${DEVICE_MODEL}_${
    BigInt(Math.trunc(Date.now() / 1000)).valueOf()
  }`;
  const suffixLength = Math.max(1, 127 - devicePrefix.length);
  const deviceId = `${devicePrefix}_${uuidGen(8).substring(0, suffixLength)}`;
  const registrationHash = md5(
    `${username}${passwordHash}${deviceId}${DIGI_APP_MARKER}${userHash}`,
  );
  return { deviceId, registrationHash };
}

function digiHeaders(): Record<string, string> {
  return {
    "Accept": "application/json, text/plain, */*",
    "Authorization": DIGI_AUTHORIZATION,
...
function redact(value: string): string {
  ...
  out = out.replace(/\b[a-f0-9]{32}\b/gi, "[redacted-hash]");
  out = out.replace(
    /Kodeative_iptvro_[A-Za-z0-9_ -]{16,}/g,
    "Kodeative_iptvro_[redacted-device]",
  );
  out = out.replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [redacted]");
  out = out.replace(
    /("?(?:ssoToken|sso_token|token|bearerToken|provisionData|deviceId|passwordHash|registrationHash)"?\s*[:=]\s*)"?[^",}\s]+/gi,
    "$1[redacted]",
  );
  return out;
}
...
function providerError(
  ...
  const rawMetaError = meta.error;
  const hasProviderError = rawMetaError !== undefined &&
    String(rawMetaError) !== "0" && String(rawMetaError).trim() !== "";
  if (res.ok && !hasProviderError) return null;
  const resultMessage = pickString(result, ["message", "info", "code"]);
  const message = resultMessage ?? compactDiagnostic(data, rawBody) ??
    rawBody.slice(0, 220);
  return new HttpError(
    res.ok ? 502 : res.status,
    `Digi provider error${res.ok ? "" : ` HTTP ${res.status}`}: ${
      redact(message || "unknown error")
    }`,
  );
}
...
async function performLogin(): Promise<SessionState> {
  if (!config.user || !config.pass) {
    throw new HttpError(401, "missing DIGI_ONLINE_USER or DIGI_ONLINE_PASS");
  }
  const username = config.user;
  const passwordHash = md5(config.pass);
  const loginUrl = new URL(`${DIGI_API_BASE}/user.php`);
  ...
  const userHash = pickString(asRecord(login.data.data), ["h"]);
  if (!userHash) {
    throw new HttpError(502, "Digi login did not return a user hash");
  }
  const { deviceId, registrationHash } = generateDeviceRegistration(
    username,
    passwordHash,
    userHash,
  );
  const registerUrl = new URL(`${DIGI_API_BASE}/devices.php`);
  registerUrl.searchParams.set("action", "registerDevice");
  registerUrl.searchParams.set("i", deviceId);
  registerUrl.searchParams.set("c", registrationHash);
  registerUrl.searchParams.set("pass", passwordHash);
  registerUrl.searchParams.set("dmo", DEVICE_MODEL);
  ...
  log(`registered Digi device ${deviceId.slice(0, 20)}...`);
  return state;
}

async function ensureDeviceId(forceLogin = false): Promise<string> {
  let state = forceLogin ? null : await readJson<SessionState>(SESSION_PATH);
  if (!state?.deviceId || state.username !== config.user) {
...
function normalizeChannel(raw: Json): DigiOnlineChannel | null {
  const media = asRecord(raw.media_channel);
  const id = pickString(raw, ["id_channel", "id_stream", "id", "channel_id"]);
  const name = pickString(raw, ["channel_desc", "channel_name", "name"]);
  if (!id || !name) return null;
  const imageUrl = findImage(media.channel_logo_url) ?? findImage(media.logo) ??
    findImage(raw.logo) ?? findImage(raw.image);
  return {
    id,
    name: name.replace(/\//g, "-"),
    slug: slugify(name),
...
async function fetchChannels(): Promise<DigiOnlineChannel[]> {
  ...
  if (!res.ok) {
    throw new HttpError(
      res.status,
      `Digi channel catalog failed: HTTP ${res.status}${
        rawBody ? ` (${redact(rawBody.slice(0, 220))})` : ""
      }`,
    );
  }
  const payload = asRecord(data.data);
  const channelsRaw = Array.isArray(payload.channels_list)
    ? payload.channels_list
    : Array.isArray(data.channels_list)
    ? data.channels_list
    : [];
  return channelsRaw.map((item) => normalizeChannel(asRecord(item))).filter((
    item,
  ): item is DigiOnlineChannel => !!item);
}

async function getChannels(force = false): Promise<DigiOnlineChannel[]> {
  const cached = await readJson<
    { updatedAt: string; channels: DigiOnlineChannel[] }
  >(CHANNELS_PATH);
...
async function findChannel(
  requested: string,
): Promise<DigiOnlineChannel | null> {
...
async function resolveStream(
  ...
  const fetchStream = async (deviceId: string): Promise<Json> => {
    const url = new URL(`${DIGI_API_BASE}/streams_l_3.php`);
    url.searchParams.set("action", "getStream");
    url.searchParams.set("id_stream", channelId);
    url.searchParams.set("platform", STREAM_PLATFORM);
    url.searchParams.set("version_app", STREAM_VERSION_APP);
    url.searchParams.set("i", deviceId);
    ...
    if (!res.ok) {
      throw new HttpError(
        res.status,
        `Digi stream failed: HTTP ${res.status}${
          rawBody ? ` (${redact(rawBody.slice(0, 220))})` : ""
        }`,
      );
    }
    const providerMessage = pickString(data, ["error", "message"]);
    if (providerMessage) {
      throw new HttpError(
        502,
        `Digi stream provider error: ${redact(providerMessage.slice(0, 220))}`,
      );
    }
    return data;
  };
  let data: Json;
  try {
    data = await fetchStream(await ensureDeviceId(force));
  } catch (e) {
    if (
      force || !(e instanceof HttpError) ||
      (e.status !== 401 && e.status !== 403 && e.status !== 502)
    ) {
      throw e;
    }
    data = await fetchStream(await ensureDeviceId(true));
  }
  const stream = asRecord(data.stream);
  const manifestUrl = pickString(stream, ["abr", "url", "stream"]);
  const licenseUrl = pickString(stream, ["proxy", "licenseUrl", "drm"]);
  if (!manifestUrl) {
    throw new HttpError(
      502,
      "Digi stream response did not include a playable URL",
    );
  }
  const info: StreamInfo = {
    ...
    drm: licenseUrl
      ? {
        licenseUrl,
        headers: normalizeLicenseHeaders(),
      }
      : undefined,
...
async function ensureMp4decrypt(): Promise<void> {
  if (!mp4decryptChecked) {
    await ensureCommand(config.mp4decrypt, "DIGI_ONLINE_MP4DECRYPT");
...
async function ensurePackager(): Promise<void> {
  if (!packagerChecked) {
    await ensureCommand(config.shakaPackager, "DIGI_ONLINE_SHAKA_PACKAGER");
...
function channelRoute(channel: DigiOnlineChannel): string {
  ...
  if (req.method === "POST" && path === "/api/login") {
    try {
      ...
      return Response.json({
        ...
        deviceIdPrefix: `${state.deviceId.slice(0, 20)}...`,
      });
    } catch (e) {
      ...
      for (const channel of channels) {
        lines.push(
          `#EXTINF:-1 tvg-id="${channel.id}" tvg-logo="${
            channel.imageUrl ?? ""
          }" group-title="Digi Online",${channel.name}`,
        );
  ...
  if (req.method === "GET" && path.startsWith("/vlc/")) {
    ...
    try {
      ...
      if (filename === "index.m3u8") {
        const info = await resolveStream(channel.id);
        if (!info.drm) {
          return new Response(
...
async function load(force=false) {
  ...
  try {
    ...
    for (const ch of data.channels) {
      ...
      el.innerHTML = \`
        <div class="thumb">\${ch.imageUrl ? \`<img src="\${ch.imageUrl}" alt="">\` : "<span>Digi Online</span>"}</div>
        <div class="body">
          <div class="name"></div>
          <div class="actions">
...
document.querySelector("#login").onclick = async () => {
  ...
  try {
    ...
    setStatus("Logged in with device " + data.deviceIdPrefix);
    await load(true);
  } catch (error) {
    setStatus(error.message);
  }
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Digi Online Player</title>
...
<body>
<header id="title">Digi Online Player</header>
...
async function main() {
  ...
  titleEl.textContent = info.name || "Digi Online Player";
...
if (import.meta.main) {
  ...
  await startupChecks();
  log(`starting on http://127.0.0.1:${config.port}`);
